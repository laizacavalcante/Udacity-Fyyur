...
def search_venues():
  ...
  requested_word = request.get_json()
  print(data, requested_word)
  requested_word = requested_word.lower()

  venue_match = db.session.query(Venue.id, Venue.name).filter(Venue.name.ilike(f"%{requested_word}%")).all()
  ...
  SELECT venue.id, venue.name 
    COALESCE(
      COUNT(shows.start_time) FILTER (WHERE shows.start_time > NOW()),
    0) as noshow
    FROM venue
    JOIN shows ON shows.venue_id = venue.id
    GROUP BY venue.name, venue.id
  )
  """
...
def show_venue(venue_id):
  ...
  output_dict = {t.name: getattr(v, t.name) 
    for v in venue for t in v.__table__.columns}

  # Past shows
  past_shows = (
      db.session.query(
          Artist.id.label('artist_id'), Artist.name.label('artist_name'), 
          Artist.image_link.label('artist_image_link'), Shows.start_time
      )
      .select_from(Venue)
      .join(Shows)
      .join(Artist)
...
def shows():
  ...
  data = (
    db.session.query(Venue.id, Venue.name, Shows.start_time, 
      Artist.id, Artist.id, Artist.image_link, Shows.start_time)
    .select_from(Venue)
    .join(Shows)
    .join(Artist)
    .all()
